...
public class ServerListRVAdapter extends RecyclerView.Adapter<ServerListRVAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        holder.serverCountry.setText(serverLists.get(holder.getAdapterPosition()).getCountry());
        Glide.with(mContext)
                .load(serverLists.get(holder.getAdapterPosition()).getFlagUrl())
                .into(holder.serverIcon);

        holder.serverItemLayout.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                listener.clickedItem(holder.getAdapterPosition());
            }
        });
    }
